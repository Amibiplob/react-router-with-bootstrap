...
const Home = () => {
    ...
    return (
        <div className='grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3  2xl:grid-cols-4 gap-4 px-5 pt-5 bg-gradient-to-r from-cyan-500 to-blue-500'>
            {sportsData.map(data => (
                <div key={data.idSport} className="card bg-base-100 shadow-xl hover:bg-slate-200">
                    <figure>  <img src={data.strSportThumb} alt="" className='w-full' /></figure>
